...
export default function DeliveryNote({ isOpen, onClose, shipVias, onShipViaUpdate, onDirtyChange = () => {}, user, companyProfile = null }) {
  // ─── List state ───────────────────────────────────────────────────────────
  ...
  const handlePrint = () => {
    ...
    const itemRows = validItems.map(item => `
      <tr>
        ...
        <td style="padding:8px 10px;border-bottom:1px solid #e5e7eb;text-align:center;">${item.shipped || 0}</td>
        <td style="padding:8px 10px;border-bottom:1px solid #e5e7eb;text-align:center;">${item.backordered || 0}</td>
      </tr>`).join('')
...
<head>
  <meta charset="utf-8"/>
  <title>Delivery Note ${deliveryNoteNo}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .notes-section { font-size: 12px; color: #374151; margin-bottom: 24px; }
...
<div class="page">
  ...
  <div class="meta-row">
    ...
    <div class="doc-meta">
      <table>
        <tr><td>Delivery Note #:</td><td>${deliveryNoteNo}</td></tr>
        <tr><td>Delivery Date:</td><td>${fmtDate(deliveryDate)}</td></tr>
        <tr><td>Customer:</td><td>${selectedCustomer || ''}</td></tr>
      </table>
    </div>
  </div>
  ...
  <table class="items-table">
    <thead>
      <tr>
        ...
        <th style="width:100px;">Shipped</th>
        <th style="width:110px;">Backordered</th>
      </tr>
    </thead>
  ...
  <div class="signature-row">
    <div class="sig-field"><div class="sig-line"></div><span>Delivered by</span></div>
            ...
            {loading ? (
              ...
              <div className={styles.emptyState} style={{textAlign:'center',padding:'48px 20px'}}>
                <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'8px'}}>
                  <i className="fas fa-truck" style={{fontSize:'48px',color:'#e2e8f0'}} />
                  <h3 style={{margin:'8px 0 4px',fontSize:'18px',fontWeight:600,color:'#4a5568'}}>No Delivery Notes Yet</h3>
                  <p style={{margin:0,color:'#a0aec0',fontSize:'14px'}}>{searchTerm ? 'Try adjusting your search' : 'Create your first delivery note to get started'}</p>
                  {!searchTerm && (
                    <button
