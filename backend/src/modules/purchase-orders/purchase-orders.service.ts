...
/** Returns the company-level GRN requirement setting */
export const getGrnRequirement = async (companyId: string, client?: any): Promise<'mandatory' | 'optional'> => {
  const conn = client || pool;
  const [rows] = await conn.query('SELECT grn_requirement FROM companies WHERE id=?', [companyId]);
  if (!(rows as any[]).length) return 'optional';
  return (rows as any[])[0].grn_requirement || 'optional';
};
...
export const listPurchaseOrders = async (companyId: string, filters: any) => {
  ...
  const [rows] = await pool.query(
    `SELECT id,purchase_order_no,vendor_id,vendor_name,order_date,expected_delivery_date,status,receipt_status,grand_total,total_ordered_qty,total_received_qty,created_at FROM purchase_orders WHERE ${where} ORDER BY order_date DESC, created_at DESC LIMIT ? OFFSET ?`,
...
export const updateStatus = async (companyId: string, poId: string, userId: string, userName: string, newStatus: string, reason?: string) => {
  return withTransaction(async (client) => {
    ...
    // Submit for approval: create an approval request if a workflow rule matches.
    // Documents without matching rules simply remain in pending_approval until manually progressed.
    if (newStatus === 'pending_approval') {
      try {
        ...
        console.error('Approval rule lookup failed for PO:', err);
      }
    }
    ...
    if (newStatus === 'received') {
      ...
      for (const li of lineItems as any[]) {
        ...
        // Lock product row and get current stock + avg_cost
        const [prodRows] = await client.query(
          'SELECT current_stock, avg_cost, cost_price FROM products WHERE id=? AND company_id=? FOR UPDATE',
        ...
        // Weighted Average Cost using PO line rate
        const unitCost = parseFloat(li.rate) || 0;
        const currentAvgCost = parseFloat(prod.avg_cost) || parseFloat(prod.cost_price) || 0;
        const newAvgCost = (unitCost > 0 && newStock > 0)
          ? (currentStock > 0
        ...
        await client.query(
          'UPDATE products SET current_stock=?, avg_cost=?, updated_at=NOW() WHERE id=? AND company_id=?',
          [newStock, parseFloat(newAvgCost.toFixed(4)), li.product_id, companyId]
        );
...
/** Direct PO → Bill conversion (only allowed when grn_requirement = 'optional') */
export const convertPOToBill = async (companyId: string, poId: string, userId: string, userName: string, data: any) => {
  return withTransaction(async (client) => {
    // Enforce GRN requirement
    const grnReq = await getGrnRequirement(companyId, client);
    if (grnReq === 'mandatory') {
      throw new ConflictError('GRN is mandatory for this company. Create a Goods Received Note first.');
    }

    const [poRows] = await client.query(
      'SELECT * FROM purchase_orders WHERE id=? AND company_id=? AND deleted_at IS NULL FOR UPDATE',
      [poId, companyId]
    );
    if (!(poRows as any[]).length) throw new NotFoundError('Purchase Order');
    const po = (poRows as any[])[0];
    if (!['approved', 'partially_received', 'received'].includes(po.status)) {
      throw new ConflictError(`PO must be approved or received to convert to bill (current: ${po.status})`);
    }

    const [items] = await client.query(
      'SELECT * FROM purchase_order_line_items WHERE purchase_order_id=? ORDER BY line_number ASC',
      [poId]
    );
    ...
    const taxAmount = (items as any[]).reduce((s: number, li: any) => s + parseFloat(li.tax_amount || 0), 0);
    const totalAmount = subtotal + taxAmount;

    await client.query(
      `INSERT INTO bills (company_id, bill_no, vendor_id, vendor_name, bill_date, due_date, status, payment_status,
        subtotal, tax_amount, discount_amount, total_amount, amount_paid, amount_due, notes, created_by, updated_by)
       VALUES (?,?,?,?,?,?,'approved','unpaid',?,?,0,?,0,?,?,?,?)`,
      [companyId, billNo, po.vendor_id, po.vendor_name, billDate, dueDate,
       subtotal, taxAmount, totalAmount, totalAmount, data.notes || null, userId, userId]
    );
    ...
    for (let i = 0; i < (items as any[]).length; i++) {
      ...
      await client.query(
        `INSERT INTO bill_line_items (bill_id, company_id, product_id, description, quantity, rate,
          tax_id, tax_rate, tax_amount, line_total, sort_order)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
        [bill.id, companyId, li.product_id || null, li.description || '',
         parseFloat(li.ordered_qty) || 0, parseFloat(li.rate) || 0,
         li.tax_id || null, parseFloat(li.tax_rate) || 0, parseFloat(li.tax_amount) || 0,
         parseFloat(li.ordered_qty) * parseFloat(li.rate), i + 1]
      );
    }

    await createAuditLog({
      company_id: companyId, entity_type: 'purchase_order', entity_id: poId,
      action: 'convert', user_id: userId, user_name: userName,
      description: `Converted directly to Bill ${billNo} (GRN skipped)`
    }, client);

    return { bill_no: billNo, bill_id: bill.id };
  });
};
