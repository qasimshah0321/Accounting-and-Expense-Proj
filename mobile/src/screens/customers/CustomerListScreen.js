...
const CustomerListScreen = ({ navigation }) => {
  ...
  const customers = useMemo(() => fuzzyFilter(allCustomers, search, ['name', 'email', 'phone']), [allCustomers, search]);

  const fetchCustomers = useCallback(async () => {
    try {
      const res = await customersAPI.getAll();
      const list = res.data?.customers || res.data?.items || [];
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        contentContainerStyle={customers.length === 0 ? styles.centered : styles.list}
