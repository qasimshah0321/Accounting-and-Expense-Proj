...
const VendorStatementScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    navigation.setOptions({ title: vendorName ? `${vendorName} Statement` : 'Vendor Statement' });
  }, [vendorName, navigation]);

  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await vendorsAPI.getStatement(vendorId, startDate, endDate);
      ...
      {loading ? (
        ...
        <>
          <View style={styles.summaryRow}>
            <View style={[styles.summaryCard, { backgroundColor: '#e65100' }]}>
              <Text style={styles.summaryLabel}>Opening Balance</Text>
              <Text style={styles.summaryVal}>{fmt(opening)}</Text>
            </View>
            <View style={[styles.summaryCard, { backgroundColor: '#c62828' }]}>
              <Text style={styles.summaryLabel}>Closing Balance (Owed)</Text>
                ...
                {rows.map((t, i) => (
                  <View key={i} style={styles.txRow}>
                    ...
                    <View style={{ flex: 2 }}>
                      ...
                      <Text style={styles.txType}>{t.type || (t.debit > 0 ? 'Payment' : 'Bill')}</Text>
                    </View>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Aging Summary (Owed to Vendor)</Text>
...
const styles = StyleSheet.create({
  ...
  filterTitle: { fontSize: 14, fontWeight: '700', color: '#e65100', marginBottom: 8 },
  ...
  applyBtn: { backgroundColor: '#e65100', padding: 10, borderRadius: 8, alignItems: 'center', marginTop: 10 },
...
export default VendorStatementScreen;
